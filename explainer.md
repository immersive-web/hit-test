...
The requirement that the hit-test API is async creates some challenges. It has some ergonomics issues in the code style that arises from the promise-based call structure, but it also has a more critical flaw in the way the time discrepancy between the request and the return affect different use-cases. Currently, since we process a 3D-ray-based hit request based on the state of the world for the frame following the request, there is noticeable drift when using hit-test to create a reticle in 3D space. This drift comes from the frame delay between when the ray is generated by projecting into the world from screen space and when that ray is used to generate the hit for the reticle - the orientation of the device no longer matches the ray that was generated when the promise resolves.

The following diagram should help in understanding the underlying issue:
<img alt="Hit-test example" src="explainer-media/timing-diagram.png" style="float: left; object-fit: cover; width: 45%; height: 20em; margin-right: 1em; ">

Situation: call requestHitTest(input ray) at frame N. 
Problem: depending on the use-case, location 1 or location 2 may be the desired location of the result. Because the surface hit may be several meters away, small device rotations can result in large discrepancies between locations 1 and 2.

Use-Cases:
*   Reticle
    *   For a pixel-perfect reticle, location 2 is the correct result. We need the reticle to be rendered in frame N+1 at location 2, otherwise it will appear to lag behind the device.
    *   This means we either need to use a 2d API for reticles since 2d remains accurate from frame to frame (is there a way this could work for HMDs?), or we need to automatically correct the 3d input ray to the current device ray by adding the delta between the device transform at frame N and N+1.
*   Object placement
    *   For accurate object placement, it's actually correct to use location 1. If the placement is based on an input event / touch event, you want the object to appear in the location that it was at - for example when it was being displayed as a preview on the reticle - when the event was triggered. This means you DON'T want location 2 as that will cause the object to appear to have jumped from where you tried to place it.

It is very important to note that location 1 and 2 are both valid depending on the context of the hit request. We could take steps to ensure that either location 1 or 2 would be returned but neither is correct in all cases. Furthermore, in the code where we are computing the result, we lack the crucial context information to understand which of the cases we are dealing with.

In order to support these use-cases, a relative ray and a frame-of-reference acquired for a device or input-source can be supplied to the hit-test API. The frame-of-reference for devices and input-sources will be automatically and dynamically updated when a new frame is generated and prior to the hit-test logic. An absolute ray will be calculated based on the relative ray and the updated frame-of-reference for the device and then the hit-test results will be returned prior to the callback for the frame. This lets the API support both object placement with an absolute ray relative to a device or an absolute ray relative to an "eye-level" WebXR coordinate system.
